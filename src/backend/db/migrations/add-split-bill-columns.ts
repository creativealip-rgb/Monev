...
export async function addSplitBillColumns() {
    ...
    try {
        ...
        try {
            db.exec('ALTER TABLE debts ADD COLUMN split_group_id TEXT');
        ...
        try {
            db.exec('ALTER TABLE debts ADD COLUMN transaction_id INTEGER REFERENCES transactions(id)');
        ...
        try {
            db.exec('ALTER TABLE debts ADD COLUMN is_split_bill INTEGER DEFAULT 0');
        ...
        try {
            db.exec('CREATE INDEX IF NOT EXISTS idx_debts_split_group ON debts(split_group_id)');
        ...
        try {
            db.exec('CREATE INDEX IF NOT EXISTS idx_debts_transaction_id ON debts(transaction_id)');
